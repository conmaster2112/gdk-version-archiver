...
LIST.action = async (_) => {
   ...
   INFO(`----------------------------\n`);
   INFO(`MIRRORS:\n`);
   for await (const folder of new Bun.Glob("*").scan({ cwd: PREFERRED_PATH + "\\mirrors", onlyFiles: false })) {
      INFO(`  ${ACCENT(folder)}\n`);
   }
   INFO(`\n`);
   INFO(`TAGS:\n`)
   for await (const folder of new Bun.Glob("*").scan({ cwd: PREFERRED_PATH + "\\tags", onlyFiles: false })) {
      INFO(`  ${ACCENT(folder)}\n`);
   }
   INFO(`\n`);
   INFO(`----------------------------\n`);
