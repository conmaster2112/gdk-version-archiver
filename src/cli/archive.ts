...
ARCHIVE.action = async (_, packageName) => {
   ...
   if (existsSync(ARCHIVE_FOLDER)) {
      if (!isForced)
         return void ERROR(
            'This version is already archived, if you are sure you can use --force tag to rewrite this version.\n'
         );
      else
         OLD_FILES = new Set(
            Array.from(
               RECURSIVE_GLOB.scanSync({
                  cwd: ARCHIVE_FOLDER,
                  dot: true,
                  absolute: false,
                  onlyFiles: true,
      ...
      if (
         !(await mkdir(ARCHIVE_FOLDER, {recursive: true}).then(
            (_) => true,
            (_) => false
         ))
      )
   ...
   for await (const file of RECURSIVE_GLOB.scan({
      ...
      if (lio > 0 && !KNOWN_FOLDERS.has(folder)) {
         ...
         if (
            !EXISTS &&
            !(await mkdir(join(ARCHIVE_FOLDER, folder), {recursive: true}).then(
               (_) => true,
               (_) => false
            ))
         )
            return void ERROR(
               `FAILED TO CREATE FOLDER IN ARCHIVE PATHS, MAYBE ITS TOO LONG? ${ARCHIVE_FOLDER.length + folder.length + 1}, ${folder}\n`
            );
      ...
      await channel.push(
         (async () => {
            //LOG(`${file}\n`);
            let succeed = await copyFile(join(path, file), join(ARCHIVE_FOLDER, file)).then(
               (_) => true,
               (_) => false
            );
            if (!succeed) failedFiles.push(file);
   ...
   if (failedFiles.length) {
      return void ERROR(
         `Something went wrong when coping these specific files: \n${failedFiles.join('\n')}\n`
      );
   }
...
async function getPackageApplicationId(installationPath: string): Promise<string | null> {
   ...
   return await readFile(join(installationPath, 'AppxManifest.xml'))
      .then((_) => {
         ...
         const id = _.toString().match(
            /<Application(?:\s+[^>]*)??\s+Id\s*=\s*["']([^"']+)["']/is
         )?.[1];
         if (!id) {
