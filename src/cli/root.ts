...
const VERBOSE = new Flag('verbose', {description: 'Shows LOG messages', long: 'verbose'});
export const TAG = new ValueFlag('tag', {
   defaultValue: 'current',
   description:
      'Environment Tag under this instance operates, archiving performs tag redirection of this name',
...
export const ENVIRONMENT_INFO = (options: Parameters<Defined<Command['onFlags']>>[0]) => {
   ...
   LOG(
      `TAG:   ${ACCENT(options.getValue(TAG))} ${options.getValue(TAG) === TAG.defaultValue ? '(default)' : ''}\n`
   );
   LOG(`-----------------------\n`);
};
